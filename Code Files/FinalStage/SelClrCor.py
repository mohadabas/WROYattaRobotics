...
class ColorsCoordinations:
    ...
    def getcoor2(self,l01,l02,l03,u01,u02,u03,l1,l2,l3,u1,u2,u3):
        for frame in self.camera.capture_continuous(self.rawCapture, format="bgr", use_video_port=True):
	# grab the raw NumPy array representing the image, then initialize the timestamp
            # and occupied/unoccupied text
            imageFrame = frame.array
            ...
            lower2 = np.array([l01, l02, l03], np.uint8) 
            upper2 = np.array([u01, u02, u03], np.uint8) 
            ...
            # For red color 
            mask = cv2.dilate(full_mask, kernal) 
            red = cv2.bitwise_and(imageFrame, imageFrame, 
                                    mask = mask) 
