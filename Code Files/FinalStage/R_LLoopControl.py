...
def forward_c(xb,yb,xo,yo):
    ...
    if(yb<yo):
        ...
        print("no color")
def forward_cR(x,min_x,max_x,Left):
    dth=0
    ath=0
    cth=0
    ...
    cur_angel=start_HA
    print(cur_angel)
    print(int(sensL.Distance_test()))
    while (int(sensF.Distance_test())>10 or (int(sensF.Distance_test())>10 and int(sensL.Distance_test()) < 120 and int(sensR.Distance_test()) < 120)):
        print(cth)

        dr=int(sensR.Distance_test())
        dl=int(sensL.Distance_test())
        if(x!=-1 and Left):
            cth=-int(((max_x-x)/max_x)*30)
        elif(x!=-1 and not Left):
            cth=int(((max_x-x)/max_x)*30)
        else:
            cth=0
    ...
    return df
def forward_cG(x,min_x,max_x,Left):
    dth=0
    ath=0
    cth=0
    ...
    while (int(sensF.Distance_test())>10 or (int(sensF.Distance_test())>10 and int(sensL.Distance_test()) < 120 and int(sensR.Distance_test()) < 120)):
        ...
        servo.angle = serv_angle
        motor.forward(0.5)
        x,y,w,h=a.getcoor(40, 55, 80,80, 255, 210)
    df=int(sensF.Distance_test())
    return df
